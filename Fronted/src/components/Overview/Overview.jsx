...
import FocusTimer from "../Pages/Focus/Focus";
...
export function Overview() {
  ...
  const weekTotaTtask = thisWeekTasks.length ;   // 0
  const weekCompletedTask = thisWeekTasks.filter((task) => {
    return task.completed == true;
  });
  
  // find percentage for completed task week
  const percentage =weekCompletedTask.length === 0 ?
 0 : 100 * (weekCompletedTask.length / weekTotaTtask)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border">
        ...
        <p className="text-md  text-redp-500 mt-1">
